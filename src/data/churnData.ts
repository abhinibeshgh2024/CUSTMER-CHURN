...
// Representative subset of the original dataset to maintain a clean codebase while preserving the true statistical properties:
// - Churn Rate: ~26.5%
// - High Fiber Optic Churn
// - High Month-to-Month Contract Churn
// - Balanced Gender Split
const rawCsv = `customerID,gender,SeniorCitizen,Partner,Dependents,tenure,PhoneService,MultipleLines,InternetService,OnlineSecurity,OnlineBackup,DeviceProtection,TechSupport,StreamingTV,StreamingMovies,Contract,PaperlessBilling,PaymentMethod,MonthlyCharges,TotalCharges,Churn
...
// Helper to parse the CSV at runtime securely and expand to exactly 1000 customers
export function parseChurnData(): Customer[] {
  const lines = rawCsv.trim().split("\n");
  const headers = lines[0].split(",");
  const dataLines = lines.slice(1);

  const parsedBase = dataLines.map((line) => {
    const values: string[] = [];
    let currentVal = "";
    let insideQuote = false;

    // A simple CSV cell parser to handle commas inside quotes if any
    ...
    headers.forEach((header, index) => {
      ...
      if (header === "SeniorCitizen") {
        record[header] = parseInt(val, 10) || 0;
      } else if (header === "tenure") {
        record[header] = parseInt(val, 10) || 0;
      } else if (header === "MonthlyCharges") {
        record[header] = parseFloat(val) || 0;
      } else if (header === "TotalCharges") {
        const parsedCharges = parseFloat(val);
        record[header] = isNaN(parsedCharges) ? 0 : parsedCharges;
      } else {
        record[header] = val;
      }
    });

    if (record.tenure === 0 && record.TotalCharges === 0) {
  ...
  // Replicate and generate deterministic variations up to exactly 1000 customer records
  const targetCount = 1000;
  const expanded: Customer[] = [];

  for (let i = 0; i < targetCount; i++) {
    const baseItem = parsedBase[i % parsedBase.length];
    
    if (i < parsedBase.length) {
      // Keep original first 147 records exactly as is
      expanded.push(baseItem);
    } else {
      // Create a deterministic variation of the base item to preserve statistics
      const index = i;
      const seed = index * 4567;
      ...
      // Tweak ID
      const parts = baseItem.customerID.split('-');
      const customerID = `${parts[0] || "XXXX"}-${String(10000 + index).slice(1)}`;
      
      // Tweak Tenure length (-3 to +3 months)
      const tenureTweak = rand(7) - 3;
      const tenure = Math.max(1, Math.min(72, baseItem.tenure + tenureTweak));

      // Tweak Monthly Charges (-4 to +4 dollars)
      const chargesTweak = (rand(80) - 40) / 10;
      const MonthlyCharges = Math.max(18.5, Math.min(119.5, parseFloat((baseItem.MonthlyCharges + chargesTweak).toFixed(2))));
      
      // Calculate Total Charges matching new tenure and charges
      const TotalCharges = parseFloat((MonthlyCharges * tenure).toFixed(2));

      // Deterministic slight feature variation (keep ~same statistics)
      const gender = rand(10) < 5 ? "Female" : "Male";
      const SeniorCitizen = baseItem.SeniorCitizen === 1 ? (rand(10) < 8 ? 1 : 0) : (rand(10) < 1 ? 1 : 0);
...
export const churnData: Customer[] = parseChurnData();
